      ...
      <div className="flex flex-1 overflow-hidden" style={{ minHeight: 0 }}>
        ...
        <div className="flex-1 flex flex-col bg-[#0E0E12] overflow-hidden min-w-0 p-3">
          ...
          <div className="flex-1 relative rounded-lg overflow-hidden border border-white/[0.08]" style={{ minHeight: 0 }}>
            ...
            {/* Plain iframe — no chrome, fills the viewfinder box */}
            <iframe
        ...
        <div style={{ width:260, flexShrink:0, borderLeft:'1px solid rgba(255,255,255,0.07)', background:'#0E0E12', display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          {/* Score header — large ring + score + Fix All */}
          <div style={{ padding:'10px 12px', borderBottom:'1px solid rgba(255,255,255,0.07)', flexShrink:0 }}>
            {scanResult ? (
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                  ...
                  <div style={{ position:'relative', width:56, height:56, flexShrink:0 }}>
                    ...
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <span style={{ fontSize:13, fontWeight:900, color:'#fff' }}>{scanResult.score}</span>
                    </div>
                  </div>
                  {/* Score text + Fix All */}
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:14, fontWeight:800, color: scanResult.pct >= 80 ? '#4ADE80' : scanResult.pct >= 60 ? '#FFC72C' : '#EF4444', lineHeight:1.2 }}>
                      {scanResult.score} / {scanResult.total} — {scanResult.pct}%
                    </div>
                    <div style={{ fontSize:10, color:'#9ca3af', marginTop:2 }}>{scanResult.checks.filter((c) => !c.pass).length} checks failing</div>
                  </div>
                  <button onClick={handleFixAll} disabled={building || !apiKey || !scanHtml}
                    style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 12px', borderRadius:8, fontSize:11, fontWeight:800, background:'#1a1a22', border:'1px solid rgba(255,204,0,0.3)', color:'#FFC72C', cursor:'pointer', flexShrink:0, whiteSpace:'nowrap', opacity:(building || !apiKey || !scanHtml) ? 0.4 : 1 }}>
                    <Zap style={{ width:13, height:13 }} /> Fix All
                  </button>
                </div>
              ...
              <div style={{ textAlign:'center', padding:'12px 0' }}>
                ...
                <div style={{ fontSize:10, color:'#6b7280' }}>Generate or scan a page to see score</div>
              </div>
            )}
          </div>

          {/* Check rows — category headers + pass/fail rows with large Fix buttons */}
          <div style={{ overflowY:'auto', maxHeight:240, flexShrink:0 }}>
            {scanResult && CAT_ORDER.map((cat) => {
              ...
              return (
                <div key={cat}>
                  ...
                  {checks.map((c) => (
                    <div key={c.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 10px', borderBottom:'1px solid rgba(255,255,255,0.03)', background: !c.pass ? 'rgba(239,68,68,0.03)' : 'transparent' }}>
                      {c.pass
                        ? <Check style={{ width:11, height:11, color:'#4ade80', flexShrink:0 }} />
                        : <X style={{ width:11, height:11, color:'#f87171', flexShrink:0 }} />}
                      <span style={{ fontSize:9, lineHeight:1.3, flex:1, color: c.pass ? '#9ca3af' : '#e5e7eb', fontWeight: c.pass ? 400 : 500 }}>{c.label}</span>
                      {!c.pass && (
                        <button
                          onClick={() => handleFixCheck(c.id)}
                          disabled={building || !apiKey || !scanHtml || fixingCheckId === c.id}
                          style={{ display:'flex', alignItems:'center', gap:3, padding:'4px 10px', borderRadius:6, fontSize:10, fontWeight:800, background:'#111', border:'1px solid rgba(255,255,255,0.12)', color:'#fff', cursor:'pointer', flexShrink:0, whiteSpace:'nowrap', opacity:(building || !apiKey || !scanHtml) ? 0.4 : 1 }}>
                          {fixingCheckId === c.id ? <Loader2 style={{ width:10, height:10 }} className="animate-spin" /> : <Zap style={{ width:10, height:10, color:'#FFC72C' }} />}
                          Fix
                        </button>
                      )}
                    </div>
