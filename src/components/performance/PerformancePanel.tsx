...
export default function PerformancePanel() {
  ...
  const handleGscPull = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch('https://www.googleapis.com/webmasters/v3/sites/sc-domain%3Agenerationhealth.me/searchAnalytics/query', {
        method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ startDate: start.toISOString().split('T')[0], endDate: end.toISOString().split('T')[0], dimensions: ['query'], rowLimit: 50 }),
      });
      if (resp.ok) { const data = await resp.json(); setGscData({ ...data, fetchedAt: new Date().toISOString(), dateRange: '28' }); }
    } catch (e) { alert('GSC pull failed: ' + (e instanceof Error ? e.message : String(e))); }
  }, [ga4Token, handleGscConnect, setGscData]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* GSC/GA4 Connect */}
      {showGscConnect && <div className="card p-5 space-y-3"><div className="text-[11px] font-bold text-gh-text-muted uppercase tracking-widest">GSC / GA4 Connection</div><div className="flex gap-3"><button onClick={handleGscConnect} className="px-4 py-2 rounded-xl text-xs font-bold bg-carolina text-white">{ga4Token ? '✓ Connected — Reconnect' : 'Connect Google OAuth'}</button>{ga4Token && <button onClick={handleGscPull} className="px-4 py-2 rounded-xl text-xs font-bold border border-carolina/30 text-carolina">Pull GSC Data (28d)</button>}</div>{ga4Token && <div className="text-[10px] text-emerald-400">OAuth token active</div>}</div>}
