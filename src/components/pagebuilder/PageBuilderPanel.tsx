                ...
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                  <div style={{ position:'relative', width:52, height:52, flexShrink:0 }}>
                    ...
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <span style={{ fontSize:11, fontWeight:800, color:'#fff' }}>{scanResult.score}</span>
                    </div>
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13, fontWeight:800, color: scanResult.pct >= 80 ? '#4ADE80' : scanResult.pct >= 60 ? '#FFC72C' : '#EF4444', lineHeight:1.2 }}>{scanResult.score} / {scanResult.total} — {scanResult.pct}%</div>
                    <div style={{ fontSize:9, color:'#9ca3af', marginTop:2 }}>{scanResult.checks.filter((c) => !c.pass).length} checks failing</div>
                  </div>
