...
async function fetchPageHTML(slug: string): Promise<string | null> {
  ...
  try {
    ...
    if (resp.ok) { const html = await resp.text(); if (html.length > 500) return html; }
  } catch { /* CORS blocked */ }

  // Strategy 2: corsproxy.io
  try {
    const proxyResp = await fetch(`https://corsproxy.io/?${encodeURIComponent(pageUrl)}`);
    if (proxyResp.ok) { const html = await proxyResp.text(); if (html.length > 500) return html; }
  } catch { /* try next */ }
  ...
  // Strategy 4: WP REST API (content only)
  try {
    ...
    if (wpResp.ok) {
      ...
      if (Array.isArray(pages) && pages.length > 0 && pages[0]?.content?.rendered) return pages[0].content.rendered;
    }
  } catch { /* exhausted */ }
...
export default function PageBuilderPanel() {
  ...
  const injectNepqBlocks = useCallback(() => {
    ...
    zonesWithHeadings.slice().reverse().forEach((z) => {
      ...
      const blockHtml = `<div class="gh-nepq-block" style="padding:28px 32px;background:#F0FDFA !important;border-left:4px solid #0D9488;border-radius:8px;margin:32px 0;color:#1A2332 !important"><p style="font-size:17px;font-weight:700;font-style:italic;color:#1A2332 !important;margin:0 0 12px;line-height:1.6">\u201c${d.q}\u201d</p><p style="font-size:17px;line-height:1.78;color:#3A4553 !important;margin:0">${d.a}</p></div>`;
